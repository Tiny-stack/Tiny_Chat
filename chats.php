...
$profile_pic=get_profile_pic_by_id($user_id);
echo '<input type="hidden" id="user_name" value="'.$name.'">';
echo '<input type="hidden" id="profile_pic" value="'.$profile_pic.'">';
echo '<input type="hidden" id="sent_to" value="0">';
?>
		...
		<div class="row h-100">
			...
			<div class="d-none d-sm-flex flex-column col-12 col-sm-7 col-md-8 p-0 h-100" id="message-area">
				...
				<div class="d-none justify-self-end align-items-center flex-row" id="input-area">
					...
					<i class="fas fa-paper-plane text-muted px-3" style="cursor:pointer;" onclick="send_msg()"></i>
				</div>
			</div>
		</div>
	</div>
...
//generateMessageArea(messages,1);
let viewChatList = () => {
	...
	$.get('backend/chat_list.php',function(result)
    {
        ...
        $.each(result.chat_list,function(idx)
        {
				...
				<div class="chat-list-item d-flex flex-row w-100 p-2 border-bottom unread" onclick="generateMessageArea(this,`+this.user_ID+`)">
				<img src="`+this.profile_pic+`" alt="profile.png" class="img-fluid rounded-circle mr-2" style="height:50px;">
				<div class="w-50">
				<div class="name">`+this.user_name+`</div>
				<div class="small last-message">last mess<i class=\"" + statusClass + " fa-check-circle mr-1\"></i>"msg.body</div>
				</div>
				<div class="flex-grow-1 text-right">
				<div class="small time">12/dec/2021</div> "<div class=\"badge badge-success badge-pill small\" id=\"unread-count\""></div>
				</div>
			</div>
			`;
		});
...
let addMessageToMessageArea = (msg) => {
	...
	let htmlForGroup = `
	<div class="small font-weight-bold text-primary">Tiny
	</div>
	`;

	let sendStatus = `<i class="far fa-check-circle"></i>`;
	...
	<div class="align-self-end self p-1 my-1 mx-3 rounded bg-white shadow-sm message-item">
		...
		<div class="d-flex flex-row">
			<div class="body m-1 mr-2">This is a message</div>
...
let generateMessageArea = (elem,chatIndex) => {
	...
	//alert("getting Info");
	$.post('backend/get_info.php',{'sent_to' :chatIndex},function(result)
	{
		//alert("I am here");
		if(!result.success)
		{
		...
		DOM.messageAreaPic.src = result.profile_pic;
	});
	
	// Message Area details ("last seen ..." for contacts / "..names.." for groups)
		DOM.messageAreaDetails.innerHTML = `last seen today`;
	...
	$('#sent_to').val(chatIndex);
	alert($('#sent_to').val());
	$.post('backend/chat_get.php',{'sent_to' :chatIndex},function(result)
	{
		if(!result.success)
		{
			console.log("ERROR FOR NEW:::");
...
let init = () => {
	DOM.username.innerHTML = $('#user_name').val();
	DOM.displayPic.src = $('#profile_pic').val();
...
function send_msg()  //Working of send message button 
{
	//event.preventDefault();
	//var url = $('#input').attr('id');
...
function pollserver() // this function will be repeatedly call on this page so that we can get new chats 
{
	//alert('pollserver is called');
	$.post('backend/chat_get.php',{'sent_to' :$('#sent_to').val()},function(result)    //getting messages from chats
	{
		if(!result.success)
		{
			return;
